...
class Settings {
  ...
  async handleResults () {
    ...
    this.log.debug(`handleResults: Full context payload: ${JSON.stringify(payload)}`)

    // Create a checkrun in both nop and non-nop mode
    this.log.debug(`handleResults: Running in nop mode: ${this.nop}`)
    ...
    */
    ...
    this.log.debug(`CREATE_PR_COMMENT environment variable value: ${env.CREATE_PR_COMMENT}`)
    if (env.CREATE_PR_COMMENT === 'true') {
      this.log.debug(`Attempting to create PR comment. Check run payload: ${JSON.stringify(payload?.check_run)}`)
      this.log.debug(`Check suite pull requests: ${JSON.stringify(payload?.check_run?.check_suite?.pull_requests)}`)
      this.log.debug(`Full context payload: ${JSON.stringify(payload)}`)
...
`
      ...
      // Get the pull request from either the check run or the context
      const pullRequest = payload.check_run.check_suite.pull_requests[0] || payload.check_suite.pull_requests[0]

      if (!pullRequest) {
        this.log.debug('No pull request found in payload, skipping PR comment creation')
        return
      }
